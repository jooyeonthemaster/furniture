  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        const bookProducts = allProducts.filter(product => 
          product.name.includes('책') || 
          product.name.includes('북') || 
          product.name.includes('잡지') || 
          product.name.includes('매거진')
        );
        ...
        <div className="container mx-auto">
          {products.length === 0 ? (
            <div className="text-center py-16">
              <p className="text-lg opacity-60">등록된 도서가 없습니다.</p>
            </div>
          ) : (
